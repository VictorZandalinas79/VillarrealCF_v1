...
class CampoFutbolGraficos:
        ...
        for jugador in filtered_df['Alias'].unique():
            ...
            if avg_minutes >= min_avg_minutes:
                ...
                accumulated_record = {
                    ...
                    # Distancias: suma total (con manejo de NaN)
                    'Distancia Total': jugador_data['Distancia Total'].fillna(0).sum(),
                    'Distancia Total 14-21 km / h': jugador_data['Distancia Total 14-21 km / h'].fillna(0).sum(),
                    'Distancia Total >21 km / h': jugador_data['Distancia Total >21 km / h'].fillna(0).sum(),
                    'Distancia Total >24 km / h': jugador_data.get('Distancia Total >24 km / h', pd.Series([0])).sum(),
    ...
    def group_players_by_demarcation(self, filtered_df):
        ...
        # Grupo 3: RESTO DE DEMARCACIONES (individuales)
        demarcaciones_combinadas = [
            ...
            'Centrocampista - MC Organizador',  # 🔥 AÑADIR
            'Sin Posición'                      # 🔥 AÑADIR
        ]
    ...
    def create_position_graph(self, players_list, demarcacion, x, y, ax, team_colors, team_logo=None):
        ...
        if num_players < 2:
            # Si solo hay 1 jugador, mostrar como punto único
            player = players_list[0]
            ax.text(x, y, f"{player['Alias']}\nDist: {player.get('Distancia Total', 0):.0f}", 
        ...
        for i, (x_pos, player) in enumerate(zip(x_positions, players_list)):
            ...
            # Mostrar dorsal y nombre
            display_text = f"{dorsal}\n{player_name}"
            
            ax.text(x_pos, y_min - 1.5, display_text, 
                    fontsize=7, color='white', weight='bold',  # 🔥 AUMENTADO de 4 a 7
    ...
    def create_visualization(self, equipo_rival, jornadas, figsize=(24, 16)):
        ...
        print(f"🔄 Creando {len(villarreal_by_demarcation)} gráficos para Villarreal CF")
        ...
        print(f"🔄 Creando {len(rival_by_demarcation)} gráficos para {equipo_rival}")
