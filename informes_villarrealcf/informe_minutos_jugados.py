...
class MinutosJugadosReport:
    ...
    def plot_stacked_bars(self, ax, table_data, jornadas):
        """Dibuja el gráfico de barras apiladas por jornada"""
        # Normalizar jornadas
        normalized_jornadas = []
        for jornada in jornadas:
            if isinstance(jornada, str) and jornada.startswith('J'):
        ...
        for i, (total, jugador) in enumerate(zip(left_values, jugadores_ordenados)):
            if total > 0:
                ...
                ax.text(total + 2, i, str(int(total)), 
                       va='center', fontsize=8, color='#2c3e50', weight='bold')
                
                # Añadir el nombre del jugador al principio de la barra, ligeramente más arriba
                ax.text(2, i + 0.2, jugador,  # Ajustado el valor de 'i' para subir el texto
                       va='bottom', ha='left', fontsize=9, color='#2c3e50',
                       bbox=dict(facecolor='white', alpha=0.7, edgecolor='none', pad=1, boxstyle='round,pad=0.1'))
