    ...
    for (let row = 0; row <= Math.min(30, range.e.r); row++) {
        for (let col = 0; col <= Math.min(20, range.e.c); col++) {
            ...
            if (cell && typeof cell.v === 'string') {
                ...
                if (valor.includes('escenarios de máxima exigencia')) {
                    const match = cell.v.match(/Escenarios de Máxima Exigencia\s+(.*)/i);
                    if (match && match[1]) {
...
function procesarArchivoXlsx(archivoPath) {
    ...
    try {
        ...
        const headersFiltered = headers.slice(1);
        
        const datosProcessed = jsonData.slice(1).map(row => {
            const rowFiltered = row.slice(1);
            const obj = {};
...
async function main() {
    console.log('🚀 Iniciando procesamiento incremental de archivos XLSX...');
    ...
    if (exitoEscritura) {
        ...
        if (datosNuevos.length > 0) {
            ...
            Object.entries(resumenNuevos).forEach(([partido, count]) => {
